...
public interface IJIDispatch extends IJIComObject
{
     ...
     * this call is cached
     * for further usage and no network call is performed again for the same
     * method name. <br>
     * 
     * @param apiName
     *            Method name.
     ...
     *             if the <code>apiName</code> is <code>null</code> or empty.
     */
     ...
     * corresponding set of <code>DISPIDs</code>.
     * The result of this call is cached for further usage and no network call
     * is performed again for the same method[argument] set.
     * 
     * @param apiName
     *            String[] with first index depicting method name and the rest
     ...
     * 
     * @param dispId
     *            <code>DISPID</code> of the method to invoke.
     * @param inparam
     ...
     * Internally it will first do a {@link #getIDsOfNames(String)} and then
     * delegates the call to {@link #put(int, JIVariant)}.
     * 
     * @param name
     ...
     * @throws JIException
     * @throws IllegalArgumentException
     *             if the <code>name</code> is <code>null</code> or empty.
     */
     ...
     * <code>name</code> parameter.
     * Internally it will first do a {@link #getIDsOfNames(String)} and then
     * delegates the call to {@link #putRef(int, JIVariant)}.
     * 
     * @param name
     *            name of the method to invoke.
     * @param inparam
     ...
     *            <code>DISPID</code> of the method to invoke.
     * @param inparams
     *            members of this array are implicitly converted to
     *            <code>JIVariant</code>s before performing the
     *            actual call to the COM server, via the
     *            <code>IJIDispatch</code> interface.
     ...
     * <code>name</code> parameter.
     * Internally it will first do a {@link #getIDsOfNames(String)} and then
     * delegates the call to {@link #get(int, Object[])}.
     * 
     * @param name
     *            name of the method to invoke.
     * @param inparams
     *            members of this array are implicitly converted to
     *            <code>JIVariant</code>s
     ...
     * <code>name</code> parameter.
     * Internally it will first do a {@link #getIDsOfNames(String)} and then
     * delegates the call to {@link #get(int)}
     * 
     * @param name
     *            name of the method to invoke.
     * @return JIVariant result of the call.
     ...
     * <code>name</code> parameter.
     * Internally it will first do a {@link #getIDsOfNames(String)} and then
     * delegates the call to {@link #callMethod(int)}. <br>
     * 
     * @param name
     *            name of the method to invoke.
     * @throws JIException
     ...
     * <code>name</code> parameter.
     * Internally it will first do a {@link #getIDsOfNames(String)} and then
     * delegates the call to {@link #callMethodA(int)}. <br>
     * 
     * @param name
     *            name of the method to invoke.
     ...
     * <code>name</code> parameter.
     * Internally it will first do a {@link #getIDsOfNames(String)} and then
     * delegates the call to {@link #callMethod(int, Object[])}. For the
     * <code>inparams</code> array, sequential <code>DISPID</code>s
     ...
     * <code>name</code> parameter.
     * Internally it will first do a {@link #getIDsOfNames(String)} and then
     * delegates the call to {@link #callMethodA(int, Object[])}. For the
     * <code>inparams</code> array, sequential <code>DISPID</code>s
     * (zero based index) will be used. For <code>inparam[0]</code> ,
     * <code>DISPID</code> will be <code>0</code>,
     * for <code>inparam[1]</code> it will be <code>1</code> and so on. <br>
     ...
     * <code>name</code> parameter.
     * Internally it will first do a {@link #getIDsOfNames(String)} and then
     * delegates the call to {@link #callMethod(int, Object[], int[])}. For the
     * <code>inparams</code> array, the corresponding <code>DISPID</code>s are
     * present in the <code>dispIds</code> array. The size of both arrays should
     * match. <br>
     * 
     * @param name
     *            name of the method to invoke.
     ...
     *            <code>IJIDispatch</code> interface.
     * @param dispIds
     *            array of <code>DISPID</code>s , matching by index to those in
     ...
     * <code>DISPID</code>s are present in
     * the <code>dispIds</code> array. The size of both arrays should match. <br>
     * 
     * @param dispId
     *            <code>DISPID</code> of the method to invoke.
     * @param inparams
     ...
     * <code>name</code> parameter.
     * Internally it will first do a {@link #getIDsOfNames(String)} and then
     * delegates the call to {@link #callMethodA(int, Object[], int[])}.For the
     * <code>inparams</code> array, the corresponding <code>DISPID</code>s are
     * present in the <code>dispId</code> array. The size of both arrays should
     * match. <br>
     * 
     * @param name
     *            name of the method to invoke.
     ...
     * <code>dispId</code> parameter.
     * For the <code>inparams</code> array, the corresponding
     * <code>DISPID</code>s are present in the <code>dispIds</code> array. The
     * size of both arrays should match.
     * 
     * @param dispId
     *            <code>DISPID</code> of the method to invoke.
     * @param inparams
     ...
     * Performs a <code>method</code> call for the method identified by the
     * <code>name</code> parameter.
     * Internally it will first do a {@link #getIDsOfNames(String[])} by forming
     * <code>name + paramNames []</code>,
     * and then delegates the call to {@link #callMethod(int, Object[], int[])}.
     * For the <code>inparams</code> array,
     * the corresponding parameter names are present in the
     * <code>paramNames</code> array. The size of both
     * arrays should match.
     * 
     * @param name
     *            name of the method to invoke.
     ...
     *            <code>IJIDispatch</code> interface.
     * @param paramNames
     *            Array of parameter names, matching by index to those in
     *            <code>inparams</code> array.
     * @throws JIException
     * @throws IllegalArgumentException
     *             if the <code>name</code> is <code>null</code> or empty.
     ...
     * Internally it will first do a {@link #getIDsOfNames(String[])} by forming
     * <code>name + paramNames []</code>,
     * and then delegates the call to {@link #callMethodA(int, Object[], int[])}
     * . For the <code>inparams</code> array,
     * the corresponding parameter names are present in the
     * <code>paramNames</code> array. The size of both
     * arrays should match.
     ...
     * <code>name</code> parameter.
     * Internally it will first do a {@link #getIDsOfNames(String)} and then
     * delegates the call to {@link #put(int, Object[])}.
     * 
     * @param name
     *            name of the method to invoke.
     * @param params
     ...
     * <code>name</code> parameter.
     * Internally it will first do a {@link #getIDsOfNames(String)} and then
     * delegates the call to {@link #putRef(int, Object[])}.
     * 
     * @param name
     *            name of the method to invoke.
     * @param params
     ...
     * {@link JIException} has been raised
     * in the last call.
     * 
     * @return
     */

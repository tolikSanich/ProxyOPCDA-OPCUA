...
public final class JIComServer extends Stub
{
    ...
    static
    {
        ...
        defaults.put ( "rpc.socketTimeout", 0);
    ...
    JIComServer ( final JISession session, final JIInterfacePointer interfacePointer, final String ipAddress ) throws JIException
    {
        ...
        super.getProperties ().setProperty ( "rpc.socketTimeout", String.valueOf(session.getGlobalSocketTimeout ()));
    ...
    private void initialise ( final JIClsid clsid, final String address, final JISession session ) throws JIException
    {
        ...
        else
        {
            ...
            /*
             * Ð’ÐºÐ»ÑŽÑ‡Ð°ÐµÐ¼ signing/sealing Ð´Ð¾ Ð²Ñ‹Ð·Ð¾Ð²Ð° init().
             * init() Ð²Ñ‹Ð¿Ð¾Ð»Ð½ÑÐµÑ‚ Ð¿ÐµÑ€Ð²Ð¸Ñ‡Ð½ÑƒÑŽ DCOM activation.
             */
            if (session.isSessionSecurityEnabled()) {
                super.getProperties().setProperty(
                        "rpc.ntlm.sign",
                        "true"
                );
        ...
        {
            logger.debug(
                    "DCOM security: sessionSecurity={}, ntlmV2={}, "
                            + "sign={}, seal={}, keyExchange={}, ntlm2={}",
                    session.isSessionSecurityEnabled(),
                    session.isNTLMv2Enabled(),
                    super.getProperties().getProperty(
                            "rpc.ntlm.sign"
                    ),
                    super.getProperties().getProperty(
                            "rpc.ntlm.seal"
    ...
    void setSocketTimeOut ( final int timeout )
    {
        ...
        getProperties ().setProperty ( "rpc.socketTimeout", String.valueOf(timeout));
    }
